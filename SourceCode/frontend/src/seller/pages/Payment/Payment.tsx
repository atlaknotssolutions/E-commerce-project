...
const Payment = () => {
    ...
    if (!jwt || sellers.reportLoaded) return;
        ...
        <div>
            <div className='grid grid-cols-1 lg:grid-cols-2 gap-3'>
                <Card className='col-span-1 p-5 rounded-md space-y-4'>
                    ...
                    <h1 className='font-bold text-xl pb-1'>₹{sellers.report?.totalEarnings ?? 0}</h1>
                    <Divider />
            ...
            <div className='mt-20'>
                ...
                <div className='flex gap-4'>
                    {tab && tab.map((item) => <Button onClick={()=>handleActiveTab(item)} variant={activeTab === item.name ? "contained" : "outlined"}>{item.name}</Button>)}
