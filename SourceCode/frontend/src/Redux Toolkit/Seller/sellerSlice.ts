...
// 🛠️ CORRECTED: fetchSellers now securely passes JWT token inside Authorization Header! [INDEX]
export const fetchSellers = createAsyncThunk<Seller[], string>(
  "sellers/fetchSellers",
  async (status: string, { rejectWithValue }) => {
    try {
      const token = localStorage.getItem("jwt"); // Dynamic token fetch from local storage [INDEX]
      ...
      const response = await api.get<Seller[]>(API_URL, {
        ...
        headers: {
          Authorization: `Bearer ${token}`, // Injected Bearer token securely! [INDEX]
        },
      });
...
// 🛠️ CORRECTED: updateSellerAccountStatus now securely passes JWT token inside Authorization Header! [INDEX]
...
// 🛠️ CORRECTED: deleteSeller now securely passes JWT token inside Authorization Header! [INDEX]
